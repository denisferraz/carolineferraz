...
$linhas = explode("\n", trim($orcamento['servicos']));

$nome = $orcamento['nome'];
$email = $orcamento['email'];
$telefone = $orcamento['telefone'];
$formas_pgto = $orcamento['formas_pgto'];
...
<div class="health-container">
    ...
    <form method="POST" action="atualizar_orcamento.php">
        ...
        <div class="form-section health-fade-in">
            ...
            <textarea class="health-input" class="textarea-custom" name="formas_pgto" rows="5" cols="43" required><?php echo htmlspecialchars(str_replace(["\\r", "\\n"], ["", "\n"], $formas_pgto)); ?></textarea>
            <textarea class="health-input" class="textarea-custom" name="observacoes" rows="5" cols="43" required><?php echo htmlspecialchars(str_replace(["\\r", "\\n"], ["", "\n"], $observacoes)); ?></textarea>
            </div></div>
            ...
            if (!preg_match('/^(.*?) \(x(\d+)\) - R\$ ([0-9\.,]+)/', $linha, $match)) continue;
            list(, $servico, $quantidade, $valor) = $match;
            $valor = str_replace('.', '', $valor); // para aceitar números como 1.234,56
            $valor = str_replace(',', '.', $valor);
            ?>
            <div class="form-row">
            <div class="health-form-group">
            <label class="health-label">Serviço/Produto</label>
            <input class="health-input" type="text" name="servico[]" value="<?= $servico ?>" required>
            </div><div class="health-form-group">
            <label class="health-label">Quantidade</label>
            <input class="health-input" type="number" name="quantidade[]" value="<?= $quantidade ?>" min="1" required>
            </div><div class="health-form-group">
            <label class="health-label">Valor Unitário</label>
            <input class="health-input" type="text" name="valor[]" value="<?= $valor/$quantidade ?>" required>
            </div><div class="health-form-group">
            <button type="button" class="health-btn health-btn-danger" onclick="this.parentElement.parentElement.remove()"><i class="bi bi-trash"></i>Excluir</button>
            </div></div>
            <?php endforeach; ?>
            
            </div>
            <input type="hidden" name="id" value="<?= $orcamento['id'] ?>">
            <button type="submit" class="health-btn health-btn-warning"><i class="bi bi-check-lg"></i>Salvar Orçamento</button>
    </form>
</div>
<script>
...
document.querySelector('form').addEventListener('submit', function(e) {
    ...
    if (servicos.length === 0) {
        servicos = document.querySelectorAll('select[name="servico[]"]');
    }
