...
if ($modelo_id) {
    // Atualiza título
    $stmt = $conexao->prepare("UPDATE modelos_prontuario SET titulo = ? WHERE token_emp = '{$_SESSION['token_emp']}' AND id = ?");
    $stmt->execute([$titulo, $modelo_id]);

    // Perguntas existentes
    $stmt = $conexao->prepare("SELECT id FROM perguntas_modelo_prontuario WHERE token_emp = '{$_SESSION['token_emp']}' AND modelo_id = ?");
    ...
    foreach ($perguntas as $ordem => $pergunta) {
        ...
        if ($pergunta_id && in_array($pergunta_id, $ids_atuais)) {
            // Atualizar
            $stmt = $conexao->prepare("UPDATE perguntas_modelo_prontuario SET ordem = ?, tipo = ?, pergunta = ?, opcoes = ? WHERE token_emp = '{$_SESSION['token_emp']}' AND id = ? AND modelo_id = ?");
            $stmt->execute([$ordem, $tipo, $texto, $opcoes, $pergunta_id, $modelo_id]);
        } else {
            // Inserir nova
            $stmt = $conexao->prepare("INSERT INTO perguntas_modelo_prontuario (modelo_id, ordem, tipo, pergunta, opcoes, token_emp) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    if ($ids_para_deletar) {
        ...
        $stmt = $conexao->prepare("DELETE FROM perguntas_modelo_prontuario WHERE token_emp = '{$_SESSION['token_emp']}' AND id IN ($in) AND modelo_id = ?");
    ...
    // Novo modelo
    $stmt = $conexao->prepare("INSERT INTO modelos_prontuario (titulo, token_emp) VALUES (?, ?)");
